...
exports.SprintMonteCarlo = void 0;
class SprintMonteCarlo {
    constructor(sprintData, simulations = 10000) {
        if (!sprintData) {
    ...
    calculateDailyVelocity() {
        var _a;
        const completedTasks = (_a = this.data.sprintItems) === null || _a === void 0 ? void 0 : _a.filter(task => task.status === "DONE" || task.status === "Concluído");
        const velocities = [];
        if (completedTasks && completedTasks.length > 0) {
            const days = new Map();
            completedTasks.forEach(task => {
                var _a;
                const date = ((_a = task.startDate) === null || _a === void 0 ? void 0 : _a.split('T')[0]) || task.startDate;
                if (date) {
    ...
    getSprintMetrics() {
        ...
        const completedTasks = this.data.sprintItems.filter(t => t.status === "DONE" || t.status === "Concluído").length;
    ...
    getCompletionStatus(probability) {
        if (probability >= 85)
            return "✅ SPRINT PROVAVELMENTE SERÁ CONCLUÍDA NO PRAZO";
        if (probability >= 50)
            return "⚠️ RISCO MODERADO DE ATRASO NA SPRINT";
    ...
    generateMarkdownReport() {
        ...
        try {
            ...
            const onTimeProb = ((_a = completionDates.find(d => d.date > sprintEndDate)) === null || _a === void 0 ? void 0 : _a.cumulativeProbability) || 100;
            ...
            const tasksByStatus = {
                "Concluído": (_b = this.data.sprintItems) === null || _b === void 0 ? void 0 : _b.filter(t => t.status === "DONE" || t.status === "Concluído").length,
                "Em Andamento": (_c = this.data.sprintItems) === null || _c === void 0 ? void 0 : _c.filter(t => t.status === "DOING" ||
                    t.status === "IN_PROGRESS" ||
                    t.status === "Em Andamento").length,
                "A Fazer": (_d = this.data.sprintItems) === null || _d === void 0 ? void 0 : _d.filter(t => t.status === "TODO" ||
                    t.status === "A Fazer").length
            };
...
exports.SprintMonteCarlo = SprintMonteCarlo;
